...
1. **List messages** — call `http_request("http://localhost:9110/slack/messages", {"days_back": 7, "max_results": 20})`
2. **Read each message** — call `http_request("http://localhost:9110/slack/messages/get", {"message_id": "<id>"})` for any that look relevant
